...
import math
from .models import *
from setup.utils import render_to_pdf, createticket

from flight.utils import createWeekDays, addPlaces, addInternationalFlights
...
FEE = 100
def index(request):
    min_date = f"{timezone.now().date().year}-{timezone.now().date().month}-{timezone.now().date().day}"
    max_date = f"{timezone.now().date().year if (timezone.now().date().month+3)<=12 else timezone.now().date().year+1}-{(timezone.now().date().month + 3) if (timezone.now().date().month+3)<=12 else (timezone.now().date().month+3-12)}-{timezone.now().date().day}"
...
def login_view(request):
    if request.method == "POST":
        ...
        if user is not None:
            login(request, user)
            # Redirect to 'next' if it's present, otherwise to 'index'
    ...
    if request.method == "POST":
        ...
        if not all([fname, lname, username, email]):
            return render(request, "register.html", {
                ...
                "continue": request.POST.get('continue', '')
            })
        ...
        try:
            ...
            return render(request, "register.html", {
                "message": "Username already taken or another error occurred.",
                "continue": request.POST.get('continue', '')
            })

        login(request, user)

        # Use continue to continue to the intended page after login
        continue_url = request.POST.get('continue') or reverse("index")
...
def query(request, q):
    ...
    return JsonResponse([{'code':place.code, 'city':place.city, 'country': place.country, 'airport': place.airport} for place in filters], safe=False)
...
def review(request):
    ...
    if request.user.is_authenticated:
        ...
        flight1ddate = datetime.strptime(date1, '%d-%m-%Y').replace(
            hour=flight1.depart_time.hour, 
        ...
        if round_trip:
            ...
            flight2ddate = datetime.strptime(date2, '%d-%m-%Y').replace(
                hour=flight2.depart_time.hour, 
        ...
        # Add the next parameter with the current URL
        params = urlencode({'next': request.get_full_path()})
        login_url = f"{reverse('login')}?{params}"
